...
// get worksheet by its title
  ...
  final Worksheet sheet = await ss.worksheetByTitle('teacher');
  //var nu = await sheet.values.allRows();
  int numb = await sheet.rowCount;
  print(numb);
  return await getTeach(4, sheet, context);
}

Future<Widget> getTeach(int nur, Worksheet sheet, BuildContext context) async {
  List<Widget> lisoft = [];
  //lisoft.add();
  int i;
  for (i = 2; i <= nur; i++) {
      ...
      Container(
        ...
        child: Column(
          children: <Widget>[
              ...
              child: CircleAvatar(
                ...
                backgroundImage: NetworkImage(
                  'https://drive.google.com/uc?export=view&id=' +
                      (await sheet.values.value(row: i, column: 2)),
                ),
              ),
            ),
            Text(
              await sheet.values.value(row: i, column: 1),
              style: TextStyle(
                fontFamily: 'Orbitron',
                fontSize: 30,
            ...
            Text(
              await sheet.values.value(row: i, column: 3),
              style: TextStyle(
                fontFamily: 'Orbitron',
                fontSize: 15,
                color: Colors.deepPurple[900],
            ...
            Text(
              await sheet.values.value(row: i, column: 4),
              style: TextStyle(
                fontFamily: 'Orbitron',
                fontSize: 15,
                color: Colors.deepPurple[900],
  ...
  lisoft.add(
    Container(
      ...
      width: MediaQuery.of(context).size.width,
      child: Text(
        'AND MANY MORE EXCELLENT AND EXPERIENCED TEACHERS.',
        style: TextStyle(
          fontFamily: 'Orbitron',
          fontSize: 20,
          color: Colors.deepPurple[900],
    ...
    lisoft.add(
      Container(
        child: RaisedButton(
          onPressed: () {
            Navigator.push(
                context, MaterialPageRoute(builder: (context) => FeedPort()));
          },
          color: Colors.deepPurple[900],
          child: Text(
            'ADD YOUR INFO',
  ...
  return SingleChildScrollView(child: Column(children: lisoft));
}

class TeachPro extends StatelessWidget {
  //static var sheet = getSheet();
  //static int nur = sheet.rowCount();
  @override
  Widget build(BuildContext context) {
    //var teachlist = getTeach(nur, sheet, context);
    return Container(
      //color: Colors.blue,
      alignment: Alignment.center,
      margin: EdgeInsets.all(10),
          ...
          if (snapshot.hasData && snapshot.data != 'non')
            ...
            return Container(
              ...
              padding: EdgeInsets.fromLTRB(
                  MediaQuery.of(context).size.width / 4.3, 0, 0, 0),
              //color: Colors.amber,
              child: Text(
